...
class Permissions extends Component {
    ...
    renderRow(route, index, header, parent) {
        ...
        cells.push(<Table.Cell key={index}><Icon className='arrow-icon' name={name}/>{route.routeName}</Table.Cell>);
        _.forEach(header, h => {
            let check = false;
            ...
            if (fix.indexOf(h.actionKey) > -1) {
                ...
                if (find) {
                    ...
                    cells.push(<Table.Cell key={index.toString()+(Math.round(Math.random()*1000)).toString() }>
                        <Checkbox
                    ...
                    cells.push(<Table.Cell key={index.toString()+(Math.round(Math.random()*1000)).toString() }>{''}</Table.Cell>);
                }
            } else {
                ...
                _.forEach(diff, d => {
                    ...
                    cells.push(<Table.Cell key={routeActionId}>
                        <Checkbox
                            route-action-id={routeActionId}
                            checked={check}
                            route-id={routeId}
        ...
        if (cLength < hLength) {
            ...
            for (let i = 0; i < sub; i++) {
                cells.push(<Table.Cell key={i.toString()+(Math.round(Math.random()*1000)).toString() }>{''}</Table.Cell>)
            }
        }
                ...
                {length === 0 ? null :
                    <Table.Row>
                        {_.map(header, (h, i) => i === 0 ?
                            <Table.HeaderCell key={i} className="cell-border-left">
                                <Popup
                            ...
                            <Table.HeaderCell key={i} className="">
                                <Popup
                                    content={h.actionName}
                                    position='top center'
                                    trigger={<Icon name={h.icon}/>}
