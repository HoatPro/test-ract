...
class RoomEdit extends Component {
    ...
    componentWillMount() {
        ...
        if (params.get('id')) {
            dispatch(roomA.initUpdate({originalUrl, action: 'update'}));
        } else {
            dispatch(roomA.initUpdate({originalUrl, action: 'insert'}));
        }
    }
    ...
    componentDidMount() {
        ...
        document.title = action === 'update' ? 'Edit Room' : 'Add new Room';
