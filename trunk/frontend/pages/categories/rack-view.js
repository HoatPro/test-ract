...
class RackView extends Component {
    ...
    componentDidMount() {
        ...
        console.log(Router.router.asPath, Router.router.query.id);
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if((action === 'insert' && loading === 2 && !_.isUndefined(current.rackId) && current.rackId !== '') || (action === 'update' && loading === 2 && _.size(current) > 0)) {
            Router.push(prevURL + '/categories/racks');
        }
        if(action === 'update' && loading === 0 && _.size(current) > 0) {
    ...
    handleBooking(e) {
        ...
        if(td) {
            ...
            Router.push(`${prevURL}/categories/booking-u`);
        }
    }
    ...
    handleAddDeviceLocal(e) {
        ...
        if(td) {
            ...
            Router.push(`${prevURL}/categories/add-device`);
        }
    }
    ...
    handleMoveU(e) {
        ...
        if(td) {
            ...
            Cookies.set('current', current);
            Router.push(prevURL + '/categories/move-u');
        }
    }
            ...
            <div>
                ...
                <DashboardLayout>
                                ...
                                <Segment>
                                    ...
                                    <Grid>
                                        <Grid.Row columns={2}>
                                            <Grid.Column>
                                                <CustomTable
                                                    header={[[{cell: <label>Pos</label>, props: {width: 1}}, 'Front', '']]}
                                                    body={data.fronts}
                                                    pagination={false}
                                                />
                                            </Grid.Column>
                                ...
                                <Segment>
                                    ...
                                    <Form>
                                        <Form.Field>
                                            ...
                                            <Progress  percent={Number(((data.used/current.uNumber)*100).toFixed(1))} inverted progress success />
                                        </Form.Field>
                                        <Form.Field>
                                            <label>Weight [{data.weight}/{current.maxWeight}]</label>
                                            <Progress  percent={Number(((data.weight/current.maxWeight)*100).toFixed(1))} inverted progress success />
                                        </Form.Field>
                                        <Form.Field>
                                            <label>Power [{data.power}/{current.maxPower}]</label>
                                            <Progress  percent={Number(((data.power/current.maxPower)*100).toFixed(1))} inverted progress success />
                                        </Form.Field>
                                    </Form>
                                </Segment>
                                <Segment>
                                    ...
                                    <Form>
                                        ...
                                        <Form.Field>
                                            <label>Total booking: {current.bookingUs? current.bookingUs.length: 0}</label>
                                        </Form.Field>
                                        <Form.Field>
                    ...
                    >
                        ...
                        <Modal.Content>
                            {   deleteType === 'Booking' ?
                                    <p>Do you want to remove the {deleteType} ({deleteData.position} - U{deleteData.positionU}): {deleteData.description} ?</p>
                                :
                                    <p>Do you want to remove the {deleteType} ({deleteData.position} - U{deleteData.positionU}): {deleteData.label} ?</p>
                            }

                        </Modal.Content>
                        <Modal.Actions>
                            <Button negative icon='remove' labelPosition='right' onClick={() => this.handleCloseModal()} content='Cancel' />
                            <Button positive icon='checkmark' labelPosition='right' content='Yes' onClick={() => this.onDeleteRack()} />
                        </Modal.Actions>
                    </Modal>
...
RackView.getInitialProps = async (context) => {
